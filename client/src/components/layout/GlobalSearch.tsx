...
export function GlobalSearch() {
  ...
  const grouped = {
    ...
    closures: results.filter(
      (r): r is Extract<SearchResult, { kind: 'closure' }> => r.kind === 'closure'
    ),
    dishes: results.filter((r): r is Extract<SearchResult, { kind: 'dish' }> => r.kind === 'dish'),
    events: results.filter(
      (r): r is Extract<SearchResult, { kind: 'event' }> => r.kind === 'event'
    ),
  };
    ...
    <div ref={containerRef} className="relative">
      <div className="relative flex items-center">
        ...
        <input
          ...
          className={cn(
            'h-8 w-48 rounded-xl border border-input bg-background md:w-64',
        ...
        {loading && (
          <span className="absolute right-2 h-3 w-3 animate-spin rounded-full border-2 border-primary border-t-transparent" />
        )}
      </div>

      {open && results.length > 0 && (
        <div className="absolute left-0 top-full z-50 mt-1.5 w-[340px] overflow-hidden rounded-xl border border-border bg-popover py-1 shadow-lg">
              ...
              {grouped.pages.map((r) => {
                ...
                return (
                  <ResultRow
                    ...
                    trailing={<CornerDownRight className="h-3.5 w-3.5 text-muted-foreground/40" />}
                  />
                );
              })}
            </SearchSection>
