...
import { formatNumber, formatPercent, formatScore, plural } from './format';
...
const COLUMNS: DataTableColumn<SatisfactionSiteRow>[] = [
  ...
  },
  {
    ...
    header: 'Réponses',
    align: 'right',
    render: (row) => formatNumber(row.votes),
...
function DishList({ rows, empty }: { rows: SatisfactionDishRow[]; empty: string }) {
  if (rows.length === 0) {
  ...
  return (
    <ul className="space-y-2">
      {rows.map((row) => (
        <li key={`${row.dish_id}`} className="flex items-baseline justify-between gap-3 text-sm">
...
export function SatisfactionView({ sites = [], siteId, onSiteClick }: SatisfactionViewProps) {
            ...
            <>
              ...
              <section className="grid gap-4 lg:grid-cols-2">
                <div className="space-y-3">
                  ...
                  <div className="rounded-xl border border-border bg-card p-4">
                    <DishList rows={data.top_dishes} empty={dishEmpty} />
                  </div>
                </div>
                <div className="space-y-3">
                  <h2 className="text-sm font-semibold text-foreground">
                  ...
                  <div className="rounded-xl border border-border bg-card p-4">
                    <DishList rows={data.flop_dishes} empty={dishEmpty} />
                  </div>
                </div>
              </section>
