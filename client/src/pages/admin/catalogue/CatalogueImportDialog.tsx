...
export function CatalogueImportDialog({
    open, categories, allTags, allCerts, onClose, onImported,
    ...
    const categoryOptions = useMemo(() => {
        ...
        for (const c of categories) {
            if (c.subcategories?.length) {
                for (const sub of c.subcategories) opts.push({ id: sub.id, label: `${c.label} › ${sub.label}` });
            } else {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            notify.success(
                ...
                skipped_count > 0 ? `${skipped_count} doublon${skipped_count > 1 ? 's' : ''} ignoré${skipped_count > 1 ? 's' : ''}` : undefined,
            );
            onImported();
            close();
        } catch {
        ...
        <Dialog open={open} onOpenChange={o => { if (!o) close(); }}>
            <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
                    ...
                    {step === 'configure' && uploadData && (
                        <div className="space-y-5">
                            <div className="space-y-1.5">
                                <Label>Catégorie des plats importés <span className="text-destructive">*</span></Label>
                                <select
                                ...
                                <p className="text-xs text-muted-foreground">Tous les plats du fichier seront rangés dans cette catégorie.</p>
                            </div>

                            <div className="space-y-1.5">
                                <Label>Colonne du nom du plat <span className="text-destructive">*</span></Label>
                                <select
                                    value={nameColumn}
                            ...
                            <div className="space-y-2">
                                <Label>Colonnes « tags & labels » <span className="text-muted-foreground font-normal">(optionnel)</span></Label>
                                <p className="text-xs text-muted-foreground">Ces colonnes seront analysées pour détecter les régimes alimentaires et certifications.</p>
                                <div className="flex flex-wrap gap-2">
                                    {uploadData.columns.filter(c => c !== nameColumn).map(col => (
                                        <button
                                            key={col}
                                            type="button"
                                    ...
                                    {uploadData.columns.filter(c => c !== nameColumn).length === 0 && (
                                        <p className="text-xs text-muted-foreground italic">Aucune autre colonne disponible.</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <label className="flex items-center justify-between gap-3 rounded-xl border border-border p-3 cursor-pointer">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Analyse le nom du plat pour suggérer les tags et labels.</p>
                                </div>
                            ...
                            <div className="flex gap-2 text-xs">
                                ...
                                {previewData.duplicate_count > 0 && (
                                    <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 font-medium">
                                        {previewData.duplicate_count} doublon{previewData.duplicate_count > 1 ? 's' : ''} ignoré{previewData.duplicate_count > 1 ? 's' : ''}
                                    </span>
                                )}
                            </div>
                                ...
                                <ul className="divide-y divide-border rounded-xl border border-border overflow-hidden">
                                    {previewData.dishes.map((dish, i) => (
                                        ...
                                        >
                                                ...
                                                {(dish.tags.length > 0 || dish.certifications.length > 0) && (
                                                    <div className="flex flex-wrap gap-1 mt-0.5">
                                                        {dish.tags.map(id => {
                                                            ...
                                                            if (!t) return null;
                                                            return (
                                                                <span key={id} className="inline-flex items-center gap-0.5 text-[11px] px-1.5 py-0.5 rounded-full bg-muted border border-border" style={{ color: t.color }} title={t.label}>
                                                        ...
                                                        {dish.certifications.map(id => {
                                                            ...
                                                            if (!c) return null;
                                                            return (
                                                                <span key={id} className="inline-flex items-center gap-0.5 text-[11px] px-1.5 py-0.5 rounded-full bg-amber-50 border border-amber-100" title={c.name}>
                                            ...
                                            {dish.is_duplicate && (
                                                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground shrink-0">Doublon</span>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                ...
                <DialogFooter className="gap-2">
                    {step === 'configure' && (
                        <Button variant="ghost" onClick={() => setStep('upload')} disabled={isLoading}>Retour</Button>
                    )}
                    {step === 'preview' && (
                        <Button variant="ghost" onClick={() => setStep('configure')} disabled={isLoading}>Retour</Button>
                    )}
                    {step === 'upload' && (
                        <Button variant="ghost" onClick={close} disabled={isLoading}>Annuler</Button>
                    )}
                    {step === 'configure' && (
                    ...
                    {step === 'preview' && (
                        <Button onClick={handleConfirm} disabled={isLoading || previewData?.new_count === 0} className="gap-1.5">
                            <Check className="w-4 h-4" />
                            {isLoading ? 'Import…' : `Importer ${previewData?.new_count ?? 0} plat${(previewData?.new_count ?? 0) > 1 ? 's' : ''}`}
                        </Button>
                    )}
                </DialogFooter>
            </DialogContent>
