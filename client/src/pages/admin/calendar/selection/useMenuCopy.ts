 ...
 */
export function useMenuCopy({ direction, restaurantId, onDone, sourceItems = [], targetDate }: UseMenuCopyParams) {
    ...
    const run = async () => {
        ...
        try {
            if (direction === 'export') {
                ...
                if (copied && skipped) {
                    notify.success(`Menu copié sur ${copied} jour${copied > 1 ? 's' : ''}, ${skipped} ignoré${skipped > 1 ? 's' : ''}`);
                } else if (copied) {
                    notify.success(`Menu copié sur ${copied} jour${copied > 1 ? 's' : ''}`);
                } else {
                    notify.info(`Aucun menu copié (${skipped} jour${skipped > 1 ? 's' : ''} ignoré${skipped > 1 ? 's' : ''})`);
                }
            } else {
