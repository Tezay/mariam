...
function ClosureScreen({ closure, canEdit, onReload }: ClosureScreenProps) {
  ...
  return (
    <div
      ...
      style={CLOSURE_HATCH_STYLE}
    >
      <div className="flex h-20 w-20 items-center justify-center rounded-full bg-card/80 shadow-sm">
        <CalendarOff className="h-10 w-10 text-muted-foreground/60" />
      </div>
      <div className="max-w-sm space-y-1 rounded-2xl bg-card/80 px-6 py-4 shadow-sm">
        ...
        <p className="pt-1 text-xs text-muted-foreground/70">
          {formatClosureRange(closure.start_date, closure.end_date)}
        </p>
      </div>
      {canEdit && (
        <Button
...
function DayDot({ dayData }: { dayData?: CalendarData[string] }) {
  ...
  return (
    <div className="mt-0.5 flex justify-center gap-0.5">
      {hasClosure && <span className="h-1 w-1 rounded-full bg-gray-400" />}
      {hasEvent && <span className="h-1 w-1 rounded-full bg-emerald-400" />}
      {hasMenu && <span className="h-1 w-1 rounded-full bg-[#093EAA]" />}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="relative flex flex-1 p-0.5">
            ...
            {(() => {
              ...
              return (
                <span
                  aria-hidden
                  className="pointer-events-none absolute bottom-0.5 top-0.5 rounded-xl bg-primary/10"
                  style={{
            ...
            {currentWeekDays.map((date) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'relative z-10 flex flex-1 flex-col items-center rounded-xl py-1.5 transition-colors',
                ...
                >
                  ...
                  <span
                    className={cn(
                      'hidden text-xs font-semibold capitalize md:block',
                      isClosure ? 'text-foreground/50' : 'text-muted-foreground'
                    )}
                  >
      ...
      {closure ? (
        ...
        <div
          key={animKey}
          className="flex flex-1 flex-col overflow-hidden duration-150 animate-in fade-in"
        >
          <AdminDayDisplay
            date={selectedDate}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Vous avez des modifications non enregistrées sur ce menu. Si vous changez de jour,
              elles seront perdues.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setPendingDate(null)}>Annuler</AlertDialogCancel>
            <AlertDialogAction
