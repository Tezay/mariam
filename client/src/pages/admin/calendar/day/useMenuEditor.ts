...
export function useMenuEditor({
...
}: UseMenuEditorOptions): UseMenuEditorReturn {
  ...
  const [originalSubstitutions, setOriginalSubstitutions] = useState<SubstitutionMap>(() =>
    subsFromMenu(menu)
  );
  ...
  const isDirty = useMemo(() => {
    ...
    if (items.some((item, i) => serializeItem(item) !== serializeItem(originalItems[i])))
      return true;
    ...
    for (const catId of allCatIds) {
      const curr = (substitutions[catId] ?? [])
        .map((d) => d.id)
        .sort()
        .join(',');
      const orig = (originalSubstitutions[catId] ?? [])
        .map((d) => d.id)
        .sort()
        .join(',');
  ...
  const save = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      setSaveError(null);
      try {
        const savedMenu = await menusApi.save(
          date,
          buildSaveItems(items),
          restaurantIdProp ?? undefined,
          chefNote
        );
        ...
        for (const [catId, dishes] of Object.entries(substitutions)) {
          ...
          const origIds = (originalSubstitutions[catIdNum] ?? [])
            .map((d) => d.id)
            .sort()
            .join(',');
          const currIds = dishes
            .map((d) => d.id)
            .sort()
            .join(',');
          if (origIds !== currIds) {
            await menusApi.updateSubstitutions(
              savedMenu.id,
              catIdNum,
              dishes.map((d) => d.id)
            );
