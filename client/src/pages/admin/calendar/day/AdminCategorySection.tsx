...
function SubstitutionPopoverButton({
  categoryId,
  editor,
}: {
  categoryId: number;
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={`flex shrink-0 items-center gap-1 rounded-lg border bg-background px-1.5 py-0.5 text-[10px] font-medium transition-colors ${
            hasSubstitutes
              ? 'border-orange-200 text-orange-500 hover:bg-orange-50 dark:border-orange-800/50 dark:hover:bg-orange-950/30'
              : 'border-border text-muted-foreground/60 hover:border-orange-200 hover:bg-orange-50 hover:text-orange-500 dark:hover:bg-orange-950/30'
          }`}
        >
      ...
      <PopoverContent className="w-72 p-3" align="end">
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-orange-500">
          Substitut si rupture
        </p>
      ...
      <div className="space-y-4">
        {directItems.length > 0 || (!hasSubcategories && canEdit) ? (
          <ItemList
            ...
            editor={editor}
            canEdit={canEdit}
            hint={hint && directItems.length > 0}
          />
        ) : null}
