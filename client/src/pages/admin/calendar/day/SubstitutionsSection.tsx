...
export function SubstituteCombobox({ categoryId, existingIds, onAdd }: {
    categoryId: number;
    existingIds: number[];
    ...
    const available = dishes.filter(d => d.category_id === categoryId && !existingIds.includes(d.id));
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button
                    type="button"
                    className="flex items-center gap-1 border border-dashed border-orange-300 rounded-full px-2 py-0.5 text-xs text-orange-500 hover:border-orange-400 hover:bg-orange-50 transition-colors"
                >
                    ...
                    <CommandList>
                        {loading && (
                            <div className="py-3 text-center text-xs text-muted-foreground">
                                <Search className="w-3.5 h-3.5 mx-auto mb-1 animate-pulse" />
                                Chargement…
                            </div>
                        )}
                            ...
                            <CommandGroup>
                                {available.map(dish => (
                                    <CommandItem
                                        key={dish.id}
                                        value={dish.name}
                                        onSelect={() => { onAdd(dish); setOpen(false); }}
...
export function SubstitutionsSection({ categoryId, editor }: { categoryId: number; editor: UseMenuEditorReturn }) {
    const dishes = editor.substitutions[categoryId] ?? [];

    const handleRemove = (dishId: number) => {
        editor.updateSubstitutions(categoryId, dishes.filter(d => d.id !== dishId));
    };
    ...
    return (
        <div>
            <div className="flex flex-wrap gap-1.5 items-center">
                {dishes.map(dish => (
                    <span key={dish.id} className="flex items-center gap-1 bg-white dark:bg-card border border-orange-200 dark:border-orange-700 rounded-full px-2 py-0.5 text-xs">
                        {dish.name}
                        <button
                            type="button"
                ...
                {dishes.length === 0 && (
                    <p className="text-xs text-muted-foreground/60 italic">Aucun substitut configuré</p>
                )}
            </div>
        </div>
    );
