...
function PreviewItemBox({
  ...
  return (
    ...
    >
      <span className="flex-1 truncate text-xs font-semibold" style={{ color: color.label }}>
        {item.dish?.name ?? '…'}
      </span>
      <button
        type="button"
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 px-4 py-10 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          Les plats que vous ajoutez apparaîtront ici.
        </p>
      </div>
    );
  }
      ...
      {nonEmptyGroups.map(({ group, color, items }) => (
        <section key={group.catId}>
          <div className="mb-1.5 flex items-center gap-2">
            ...
            >
              {group.parentLabel
                ? `${group.parentLabel} · ${group.category.label}`
