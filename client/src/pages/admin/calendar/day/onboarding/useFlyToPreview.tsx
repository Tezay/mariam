...
export function useFlyToPreview() {
    ...
    const fly = useCallback((sourceEl: HTMLElement | null, label: string, color: CategoryColor) => {
        ...
        for (const el of targetsRef.current.values()) {
            ...
            if (rect && rect.width > 0 && rect.height > 0) { target = rect; break; }
        }
        if (!target) return;
        ...
        setFlights(prev => [...prev, {
            ...
            to: { x: target.left + target.width / 2 - 20, y: target.top + target.height / 2 - 14 },
        }]);
    }, [reducedMotion]);

    const overlay = flights.length === 0 ? null : createPortal(
        <>
            {flights.map(f => (
                <motion.div
                    key={f.id}
                    className="fixed left-0 top-0 z-[60] pointer-events-none rounded-xl px-2 py-1.5 text-xs font-semibold truncate max-w-[180px]"
                    style={{
                        backgroundColor: f.color.bg,
                        color: f.color.label,
                        borderBottom: `3px solid ${f.color.border}`,
                    }}
                    initial={{ x: f.from.x, y: f.from.y, width: Math.min(f.from.width, 180), opacity: 1, scale: 1 }}
