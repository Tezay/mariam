    ...
    <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border bg-background px-3 sm:px-4">
      <button
        ...
        className={cn(
          ...
          canGoBack ? 'hover:bg-muted hover:text-foreground' : 'pointer-events-none opacity-0'
        )}
      >
      ...
      <div
        ...
        aria-valuenow={Math.min(activeIdx, groups.length)}
      >
        {groups.map((group, idx) => {
          ...
          return (
            <span
              ...
              style={
                done || active ? { backgroundColor: color.bg, opacity: done ? 0.45 : 1 } : undefined
              }
            />
          );
        })}
