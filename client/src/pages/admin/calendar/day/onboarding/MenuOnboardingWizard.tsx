...
export function MenuOnboardingWizard({
  date,
  restaurantId,
  categories,
  onDone,
  onCancel,
}: MenuOnboardingWizardProps) {
              ...
              >
                {step.kind === 'category' && step.sub === 'pick' && (
                  <ItemPickStep
                    ...
                    onSelectWithFly={handleSelectWithFly}
                  />
                )}
                {step.kind === 'category' && step.sub === 'tags' && <TagsStep state={state} />}
                {step.kind === 'category' && step.sub === 'photo' && <PhotoStep state={state} />}
                {step.kind === 'category' && step.sub === 'added' && (
                  <ItemAddedStep state={state} />
                )}
                {step.kind === 'category' && step.sub === 'substitutions' && (
                  <SubstitutionsStep state={state} />
                )}
                {step.kind === 'chef-note' && <ChefNoteStep state={state} />}
                {step.kind === 'celebration' && (
                  <CelebrationStep state={state} dateLabel={dateLabel} />
                )}
              </motion.div>
            </AnimatePresence>
          )}
      ...
      <AlertDialog open={exitDialogOpen} onOpenChange={setExitDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter className="flex-col gap-2 sm:flex-col">
            ...
            <AlertDialogCancel className="mt-0 w-full rounded-xl">Rester</AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
