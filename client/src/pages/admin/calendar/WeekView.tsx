...
import { BookOpen, CheckSquare, Copy, Eye, EyeOff, FileUp, Package, PackageX, Plus, Save, Trash2, Undo2 } from 'lucide-react';
import {
    DndContext,
    DragOverlay,
    PointerSensor,
...
export function WeekView({ weekStart, data, canEdit, serviceDays, restaurantId, categories, onReload, onDirtyChange, onEditEvent, onStartOnboarding }: WeekViewProps) {
    ...
    const [activeDragItem, setActiveDragItem] = useState<{ item: MenuItem; color: ReturnType<typeof getCategoryColor> } | null>(null);
    ...
    const dropHandlersRef = useRef<Record<string, (categoryId: number, dishId: number, dish?: DishCatalogItem) => void>>({});
    const removeHandlersRef = useRef<Record<string, (itemId: number) => void>>({});
    const updateItemHandlersRef = useRef<Record<string, (itemId: number, changes: Partial<MenuItem>) => void>>({});
    ...
    const handleDragStart = useCallback((event: DragStartEvent) => {
        const d = event.active.data.current as { item: MenuItem; color: ReturnType<typeof getCategoryColor>; categoryId: number; date?: string } | undefined;
        if (!d) return;
        ...
        const isInSelection = selection.selection.some(e => e.type === 'item' && e.itemId === draggedId);
    ...
    const handleDragEnd = useCallback((event: DragEndEvent) => {
        ...
        const dragData = active.data.current as { dishId?: number; categoryId?: number; date?: string; item?: MenuItem } | undefined;
    ...
    type ContextTarget =
        ...
        | { type: 'item'; date: string; itemId: number; menuId?: number; isOos: boolean; dishId?: number | null };
    ...
    const [deleteMenuTarget, setDeleteMenuTarget] = useState<{ menuId: number; date: string } | null>(null);
    ...
    const [pendingAddDish, setPendingAddDish] = useState<{ date: string; categoryId: number } | null>(null);
    ...
    const handleContextMenu = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (itemEl) {
            ...
            setContextTarget({ type: 'item', date: itemDate, itemId, menuId, isOos: item?.is_out_of_stock ?? false, dishId: item?.dish_id });
            return;
        }
        ...
        if (dayEl) {
            ...
            setContextTarget({ type: 'day', date, menuId: dayData?.menu?.id, menuStatus: dayData?.menu?.status ?? null });
            return;
        }
    ...
    const handleContextToggleOos = useCallback(() => {
        ...
        updateItemHandlersRef.current[contextTarget.date]?.(contextTarget.itemId, { is_out_of_stock: !contextTarget.isOos });
    }, [contextTarget]);
    ...
    const openAddDish = useCallback((categoryId: number) => {
        if (contextTarget?.type !== 'day') return;
        const { date } = contextTarget;
        setAddDishCatalog([]);
        setAddDishLoading(true);
        catalogApi.list({ category_id: categoryId, sort: 'name' })
    ...
    const contextItem = contextTarget?.type === 'item'
        ? (getItemsHandlersRef.current[contextTarget.date]?.() ?? data[contextTarget.date]?.menu?.items ?? [])
    ...
    const handleColumnsPointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
        if (!canEdit) return;
        const target = e.target as HTMLElement;
    ...
    const handleColumnsPointerUp = useCallback((_e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (!wasActive) { setRubberBand(null); return; }

        const container = columnsContainerRef.current;
        if (!container || !rubberBand) { setRubberBand(null); return; }
        ...
        if (width < 4 && height < 4) { setRubberBand(null); return; }
        ...
        itemEls.forEach(el => {
            ...
            // Intersection check
            if (elRight > selRect.left && elLeft < selRect.right && elBottom > selRect.top && elTop < selRect.bottom) {
    ...
    return (
        ...
        >
            <div className="flex flex-col flex-1 overflow-hidden">
                ...
                {canEdit && (
                    <div className={cn(
                        'flex items-center gap-2 px-4 py-1.5 border-b border-border shrink-0 transition-colors',
                        selection.selectionMode && selection.selectedItemCount > 0 ? 'bg-primary/5' : 'bg-card',
                    )}>
                                ...
                                {anyDirty && (
                                    <>
                                        ...
                                        <button
                                            ...
                                            disabled={isSavingAll}
                                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary text-white text-xs font-medium hover:bg-primary/90 transition-colors disabled:opacity-50"
                                        >
                ...
                <ContextMenu>
                    <ContextMenuTrigger asChild>
                        <div
                            ...
                            className="relative flex flex-1 overflow-auto border-t border-border select-none pb-24 sidebar:pb-0"
                    ...
                    {visibleDays.map(date => (
                        <WeekDayColumn
                            ...
                            onRegisterDrop={fn => { dropHandlersRef.current[date] = fn; }}
                            onRegisterRemove={fn => { removeHandlersRef.current[date] = fn; }}
                            onRegisterUpdateItem={fn => { updateItemHandlersRef.current[date] = fn; }}
                            onRegisterGetItems={fn => { getItemsHandlersRef.current[date] = fn; }}
                                ...
                                <ContextMenuSub>
                                    ...
                                    <ContextMenuSubContent className="w-44">
                                        {categories.filter(c => !c.parent_id).map(cat => (
                                            <ContextMenuItem key={cat.id} onClick={() => runAfterMenuClose(() => openAddDish(cat.id))}>
                                                {cat.label}
                                            </ContextMenuItem>
                                        ))}
                                        {categories.flatMap(c => c.subcategories ?? []).map(sub => (
                                            <ContextMenuItem key={sub.id} className="pl-6 text-muted-foreground" onClick={() => runAfterMenuClose(() => openAddDish(sub.id))}>
                                                {sub.label}
                                            </ContextMenuItem>
                                        ))}
                                ...
                                {contextTarget.menuId && (
                                    <ContextMenuItem className="gap-2" onClick={handleContextTogglePublish}>
                                        {contextTarget.menuStatus === 'published'
                                            ? <><EyeOff className="w-3.5 h-3.5" /> Passer en brouillon</>
                                            : <><Eye className="w-3.5 h-3.5" /> Publier</>
                                        }
                                    </ContextMenuItem>
                                )}
                                <ContextMenuSeparator />
                                <ContextMenuItem className="gap-2" onClick={() => runAfterMenuClose(() => setContextImportDate(contextTarget.date))}>
                                    <FileUp className="w-3.5 h-3.5" />
                                    Importer depuis…
                                </ContextMenuItem>
                                <ContextMenuItem className="gap-2" onClick={() => runAfterMenuClose(() => {
                                ...
                                {contextTarget.menuId && (
                                    <>
                                        ...
                                        <ContextMenuItem
                                            ...
                                            onClick={() => runAfterMenuClose(() => setDeleteMenuTarget({ menuId: contextTarget.menuId!, date: contextTarget.date }))}
                                        >
                        ...
                        {contextTarget?.type === 'item' && (
                            isBulkContext ? (
                                <>
                                    ...
                                    <ContextMenuItem className="gap-2" onClick={() => runAfterMenuClose(() => setContextDupeItems(selectedItems))}>
                                        <Copy className="w-3.5 h-3.5" />
                                        Dupliquer
                                    </ContextMenuItem>
                                    <ContextMenuSeparator />
                                ...
                                <>
                                    ...
                                    <ContextMenuItem className="gap-2" onClick={() => runAfterMenuClose(() => setContextDupeItems(contextItem ? [contextItem] : []))}>
                                        <Copy className="w-3.5 h-3.5" />
                                        Dupliquer
                                    </ContextMenuItem>
                                    {contextTarget.dishId && (
                                        <ContextMenuItem className="gap-2" onClick={() => runAfterMenuClose(() => navigate(`/admin/catalogue/${contextTarget.dishId}`))}>
                ...
                {activeDragItem && (
                    <div className={cn('relative', isCopyMode && 'cursor-copy')}>
                        ...
                        {multiDragItems.length > 1 && (
                            <span className="absolute -top-2 -right-2 bg-primary text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-sm">
                                {multiDragItems.length}
                            </span>
                        )}
                        {isCopyMode && (
                            <span className="absolute -top-2 -left-2 flex items-center gap-0.5 bg-emerald-500 text-white text-[10px] font-bold rounded-full pl-1 pr-1.5 py-0.5 shadow-sm">
            ...
            {pendingAddDish && (
                <Dialog open onOpenChange={open => { if (!open) setPendingAddDish(null); }}>
                                            ...
                                            <CommandItem
                                                ...
                                                onSelect={() => {
                                                    dropHandlersRef.current[pendingAddDish.date]?.(
                                                        pendingAddDish.categoryId, dish.id, dish,
                                                    );
            ...
            <MenuCopyPopover
                ...
                onOpenChange={o => { if (!o) setContextImportDate(null); }}
                anchorPoint={dupeAnchor}
                targetDate={contextImportDate ?? undefined}
                restaurantId={restaurantId}
                onDone={() => { setContextImportDate(null); onReload(); }}
            />

            {/* Context menu */}
            <AlertDialog open={deleteMenuTarget !== null} onOpenChange={o => { if (!o) setDeleteMenuTarget(null); }}>
                <AlertDialogContent>
                    <AlertDialogHeader>
            ...
            <MenuCopyPopover
                ...
                onOpenChange={o => { if (!o) setContextDupeItems(null); }}
                anchorPoint={dupeAnchor}
                sourceItems={contextDupeItems ?? []}
                restaurantId={restaurantId}
                onDone={() => { setContextDupeItems(null); onReload(); }}
            />
        </DndContext>
    );
}
