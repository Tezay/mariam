...
export function CalendarPage() {
  ...
  const handleNavigate = useCallback(
    (view: DesktopView | MobileView, date: string) => {
      guardedAction(() => {
        setCenterDate(date);
  ...
  const { data, isLoading, error, reload, restaurantId, serviceDays } = useCalendarData(
    rangeStart,
    rangeEnd
  );
  ...
  const canGoPrev = useMemo(() => {
    ...
    if (desktopView === 'month')
      return `${curYear}-${String(curMonth + 1).padStart(2, '0')}-01` > navMinDate;
  ...
  const canGoNext = useMemo(() => {
    ...
    if (desktopView === 'month')
      return `${curYear}-${String(curMonth + 1).padStart(2, '0')}-01` < navMaxDate;
  ...
  const handleToday = useCallback(
    () => guardedAction(() => setCenterDate(today)),
    [today, guardedAction]
  );
  ...
  return (
    <div className="flex h-[calc(100dvh-9rem)] flex-col overflow-hidden sidebar:h-[calc(100dvh-4rem)]">
      ...
      <AlertDialog
        open={isGuardDialogOpen}
        onOpenChange={(open) => {
          if (!open) handleGuardCancel();
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleGuardCancel}>Annuler</AlertDialogCancel>
            <AlertDialogAction
