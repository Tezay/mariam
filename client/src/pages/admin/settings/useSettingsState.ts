...
import {
    adminApi, inboxApi,
    DietaryTag, CertificationItem, ServiceHours, RestaurantWithConfig,
    NotifPreferences, CalendarSettings,
} from '@/lib/api';
import { validateEmail, validatePhone, serializeRestaurantState, RestaurantFormState } from './settings-shared';
...
export function useSettingsState() {
    ...
    const restaurantState: RestaurantFormState = {
        name, serviceDays, serviceHours, enabledTags, enabledCerts,
        addressLabel, addressLat, addressLon, email, phone, capacity,
        paymentMethods, pmrAccess,
    };
    ...
    const hasChanges = useMemo(() => {
        ...
        return restaurantSnapshot !== snapshotsRef.current.restaurant
            ...
            try {
                ...
                snapshotsRef.current = {
                    restaurant: serializeRestaurantState({
                        name: loadedName, serviceDays: loadedServiceDays, serviceHours: loadedServiceHours,
                        enabledTags: loadedTags, enabledCerts: loadedCerts,
                        addressLabel: loadedAddressLabel, addressLat: loadedLat, addressLon: loadedLon,
                        email: loadedEmail, phone: loadedPhone, capacity: loadedCapacity,
                        paymentMethods: loadedPayments, pmrAccess: loadedPmr,
                    }),
    ...
    const saveAll = useCallback(async (): Promise<SaveOutcome> => {
        ...
        const calendarDirty = calendarSettings !== null && calendarSnapshot !== snapshotsRef.current.calendar;
            ...
            if (restaurantDirty) {
                try {
                    ...
                    snapshotsRef.current.restaurant = serializeRestaurantState({
                        name: saved.name || name, serviceDays: savedDays, serviceHours: savedHours,
                        enabledTags: savedTags, enabledCerts: savedCerts,
                        addressLabel: savedLabel, addressLat: savedLat, addressLon: savedLon,
                        email: savedEmail, phone: savedPhone, capacity: savedCapacity,
                        paymentMethods: savedPayments, pmrAccess: savedPmr,
                    });
                } catch {
        ...
        email, phone, addressLabel, addressConfirmed, addressLat, addressLon,
        name, serviceDays, serviceHours, capacity, paymentMethods, pmrAccess,
        enabledTags, enabledCerts, prefs, calendarSettings,
        restaurantSnapshot, prefsSnapshot, calendarSnapshot,
    ]);

    return {
        isLoading, isSaving, hasChanges, saveAll,
        name, setName,
        serviceDays, setServiceDays,
        serviceHours, setServiceHours,
        sameHoursForAll, setSameHoursForAll,
        enabledTags, setEnabledTags,
        enabledCerts, setEnabledCerts,
        addressLabel, setAddressLabel,
        addressLat, setAddressLat,
        addressLon, setAddressLon,
