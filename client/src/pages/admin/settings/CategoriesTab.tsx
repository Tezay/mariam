  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Les modifications sont enregistrées immédiatement. L'étoile{' '}
          <Star className="inline h-3 w-3 text-amber-500" /> indique les catégories principales pour
          l'affichage du menu public.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {catLoading ? (
                    ...
                    <div key={cat.id} className="space-y-2">
                      ...
                      {cat.subcategories && cat.subcategories.length > 0 && (
                        <div className="mt-1">
                          ...
                          <button
                            ...
                            className="ml-8 flex items-center gap-1.5 py-1 text-sm text-muted-foreground transition-colors hover:text-foreground"
                          >
                      ...
                      {(!cat.subcategories || cat.subcategories.length === 0) &&
                        cat.is_protected && (
                          <button
                            onClick={() => handleAddSubcategory(cat.id)}
                            className="ml-8 mt-1 flex items-center gap-1.5 py-1 text-sm text-muted-foreground transition-colors hover:text-foreground"
                          >
