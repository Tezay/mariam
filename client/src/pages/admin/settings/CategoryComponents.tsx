...
export type CategoryPatch = Partial<{ label: string; is_highlighted: boolean; color_key: string | null }>;
...
export function CategoryRow({ category, onUpdate, onDelete, indent = false }: CategoryRowProps) {
    ...
    return (
        <div
            ...
            className={`bg-muted/50 rounded-xl p-2 sm:p-3 ${indent ? 'ml-4 sm:ml-8 border-l-2 border-border' : ''}`}
        >
...
export function SortableCatList({ items, onUpdate, onDelete, onReorderDone, indent }: SortableCatListProps) {
        ...
        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={localItems.map(c => c.id)} strategy={verticalListSortingStrategy}>
                <div className="space-y-2">
                    {localItems.map(cat => (
                        <CategoryRow key={cat.id} category={cat} onUpdate={onUpdate} onDelete={onDelete} indent={indent} />
                    ))}
                </div>
            </SortableContext>
        </DndContext>
