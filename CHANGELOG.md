...
- **Guided tour** on first sign-in: the dashboard, then the catalogue and the statistics on first visit. Replayable from **Mon compte**, and its state follows the account rather than the browser.
- **Second-factor enrolment**: an account left with neither TOTP nor passkey lands on a dedicated page and leaves it only by enabling one.
